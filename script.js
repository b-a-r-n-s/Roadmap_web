...
const DEFAULT_BUILT = ['pf-bmi', 'pf-tk', 'pf-easyauto'];
...
// seed realistic starting progress on first-ever run only
const SEED_COMPLETED = ['p1-variables','p1-functions','p1-loops','p1-dicts','p1-oop','p1-errors','p1-files','p2-numpy','p2-pandas'];
const SEED_MILESTONES = ['m1'];

let completed = new Set(load(LS.completed, null) || SEED_COMPLETED);
let milestonesDone = new Set(load(LS.milestones, null) || SEED_MILESTONES);
let built = new Set(load(LS.built, null) || DEFAULT_BUILT);
  ...
  return `
    <div class="module ${isDone ? 'completed' : ''}" data-module="${id}">
      <div class="module-row" data-open="${id}">
        <div class="module-check"><svg viewBox="0 0 16 16">${ICONS.check}</svg></div>
...
function milestoneHTML(id) {
  ...
  return `
    <div class="milestone ${isDone ? 'completed' : ''}" data-milestone="${id}">
      <div class="milestone-mark"><svg viewBox="0 0 24 24">${ICONS[ms.icon] || ICONS.target}</svg></div>
...
function openLesson(id, kind) {
  ...
  const isDone = kind === 'milestone' ? milestonesDone.has(id) : completed.has(id);
  ...
  document.getElementById('complete-btn').style.display = isDone ? 'none' : 'block';
  document.getElementById('already-done').style.display = isDone ? 'block' : 'none';
...
document.getElementById('complete-btn').addEventListener('click', (e) => {
  ...
  if (currentLessonKind === 'milestone') {
    milestonesDone.add(currentLesson);
  } else {
  ...
  document.getElementById('lesson-status-chip').textContent = '✓ Completed';
  document.getElementById('lesson-status-chip').classList.add('is-done');

  burstCelebration(e.clientX || window.innerWidth / 2, e.clientY || window.innerHeight / 2);
  renderPhases();
  renderStats();
  renderBadges();
...
function runBoot() {
  ...
  const lines = [
    ...
    stats.completedCount > 0 ? 'welcome back, Barns_' : 'welcome, Barns_',
  ];
  ...
  let lineIndex = 0;
  function nextLine() {
    if (lineIndex >= lines.length) { setTimeout(hide, 350); return; }
  ...
  setTimeout(hide, 2200); // hard cap so it never blocks the app
}
